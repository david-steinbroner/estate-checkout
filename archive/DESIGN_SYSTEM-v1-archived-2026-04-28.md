# DESIGN SYSTEM (v1, archived) — Estate Checkout

> **⚠ ARCHIVED — 2026-04-28**
>
> Superseded by the iOS-native design system on 2026-04-23 (originally `DESIGN_SYSTEM_V2.md`, now the canonical `DESIGN_SYSTEM.md`). Direction shifted from a Tailwind-based SaaS palette to iOS-native (Apple Wallet + Venmo references) with formalized consignor rules based on field feedback.
>
> Kept for historical context only. **Do not implement against it.** All current work follows `DESIGN_SYSTEM.md`.
...
- The CSS Rules section (§3) and Design Principles section (§4) in `CLAUDE.md` — this document is now the authoritative design reference. CLAUDE.md should reference this file.
