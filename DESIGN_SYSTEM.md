...
**Status:** **Migration complete + token gaps closed (v184).** Updated through **v216**. As of v184 every component family in the catalog has been renamed to canonical `.ec-*` naming, with three documented exceptions (`.sheet` itself — too entangled to rename cheaply; `.consignor-color-chip` — already a `.ec-picker-button` modifier; `.join-code-status` — informational status, not a pure field-error). Tokens fully cleaned up: dead tokens removed, all magic numbers tokenized (28px sheet titles → `--font-size-title-medium`, 48px sheet hero amounts → `--font-size-display`), all hardcoded shadows tokenized (`--shadow-button`, `--shadow-card` added). The Flash toast was removed entirely — it violated principles 1.1.6 (success modals) and 1.5 (errors as popups); replaced by live state updates and inline `.ec-field-error` messages.

**Notable patterns added v214–v216** (v213 field-test round-trip):
- **Price + caption pattern** (v214, refined v215, v216) — final price is the hero on the right; a small caption sits underneath in `--color-text-secondary` showing per-unit ("$0.95 each"), day-discount mention ("5% off"), and/or "was $X" / "Marked from $X". Replaced strikethrough comparison entirely (kept only on voided transactions, where it's semantically meaningful). Single source of truth: `Utils.formatItemPriceCaption(item)`. Used on cart row, order-review sheet, QR screen, payment screen, dashboard detail, customer's saved ticket. Mirrors Apple Wallet / Venmo / Cash App "show one number, breakdown one layer deeper" pattern.
- **Invoice-adjustment label helper** (v214) — `Utils.formatTicketDiscountLabel(adj)` returns `{short, long}` strings for any adjustment shape (legacy + v206+). Single source of truth across QR screen, payment screen, dashboard detail, customer's saved ticket. Killed the silent-mislabel bug class where each surface independently switched on `adj.type === 'percent'` and silently mishandled v206-shape data.
- **In-sheet numpad for sheet inputs** (v215) — Invoice Adjustment + Haggle (since-deleted) sheets retired their iOS-keyboard `<input>` for the same `.ec-numpad` Add Item uses. Hero number on top, 3×4 keypad below. Eliminates the keyboard-pushup class of bugs entirely (no system keyboard involved) and matches the rest of the design system.
- **Combined intro+permission modal** (v215) — replaces the old two-modal voice flow (separate "How to Use Voice Input" + "Allow Microphone" sheets that both fired in sequence). One short pre-permission modal that explains how voice works AND warns about the iOS system prompt. After permission grant, no second modal.
- **Listening indicator pill** (v215) — `.speech-indicator` pill (red pulsing dot + "Listening…" label) at the top of the Add Item entry screen. Visible while the user holds the mic button — the button itself gets covered by the user's finger so feedback was previously invisible.
- **Flat list rows in order-review sheet** (v216) — per-row gray-card backgrounds + inter-row gaps dropped in favor of flat rows with thin `--color-divider` bottoms (like dashboard detail rows). Sheet auto-shrinks to content. Each row gets a small `›` chevron after the price to signal "tap to edit." Sheet actions row is right-aligned so destructive links sit unobtrusively in the bottom-right.
- **Sheet-cap escalation pattern** (v216) — when a sheet's content reliably exceeds the default `max-height: 80lvh` cap on smaller phones (chips + hero + numpad + preview + 3 action buttons), bump to `95lvh` via an id-scoped override (`#ticket-discount-modal .sheet { max-height: 95lvh }`). Same approach `#consignor-modal` already uses. Don't change the global default.

**Lessons from the v214 round-trip (recorded v216):**
- Don't strip context from per-row captions just because it's also in a global pill. Workers reading a row need the WHY at the row level — repetition with a small global pill is fine, not noisy. The v215 strip ("$0.95 each" alone, no day-discount mention) had to be reverted in v216 because workers got confused seeing the discounted unit price without knowing why.
- When introducing chained CSS class selectors (e.g. `.sheet__title--editable` modifier), bump specificity (`.sheet__title.sheet__title--editable`) so source-order overrides don't silently revert the modifier. Single-class selectors lose to later same-specificity rules.
